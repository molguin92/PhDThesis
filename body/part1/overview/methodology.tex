...
Firstly, it preserves the realism of effects stemming from the hardware and network.
...
Secondly, it allows for easier scaling of studies compared to real-world studies.
...
In the case of interactive and immersive applications, scaling the number of human users can be a significant challenge, as recruiting participants is an expensive and time-consuming endeavor.
On the other hand, scaling applications involving \glspl{CPS} such as \glspl{NCS} can be prohibitively complex and expensive depending on the specific hardware involved.
...
Thirdly, in hand with the previous point, emulating the workload provides improved repeatability and replicability.
...
Finally, it improves extensibility significantly.
...
This can further be simplified by employing softwareplatforms for the emulation of categories of workloads, as well will discuss later on in this thesis.
...
\end{inlineenum}.
Three scenarios were considered, corresponding to setups with \num{1} and \num{10} clients under ideal circumstances, plus an additional scenario with \num{10} clients located at a distance from the \gls{AP} such that the signal strength was significantly degraded.
