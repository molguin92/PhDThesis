\section{Implications of the methodology in the context of \acs{WCA}}\label{sec:extendingmethodology}
...
Simulations are performed using these sampling schemes implemented in Python.
The sample-count-optimized aperiodic scheme results in a lower mean number of samples per step compared to the reference scheme, while still satisfying the expected wait time bound.
In addition, the results show that the embedded timing model significantly improves the performance of the sample-count-optimized aperiodic scheme compared to using fixed values for \ensuremath{\alpha} and \ensuremath{\beta}.
These results demonstrate the efficacy of the optimization approach, combined with our novel model of human behavior, in minimizing sampling rates in \gls{WCA} applications while still meeting responsiveness requirements.
