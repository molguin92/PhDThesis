...
More specifically, in \cref{paper:Olguin:DemoScaling2018,paper:Olguin:EdgeDroid2019} we introduce this methodology for \ac{WCA}.
\cref{paper:Olguin:DemoScaling2018} presents a short, high level overview of this methodological approach, in particular making very straightforward assumptions about human behavior in \ac{WCA} applications.
\cref{paper:Olguin:EdgeDroid2019} extends the discussion from \cref{paper:Olguin:DemoScaling2018}, providing more details on the implementation of the necessary measurement framework and the tooling employed, as well as on the assumptions that were made about human behavior and reactions.
...
\cref{paper:Olguin:EdgeDroid2019} furthermore presents the first empirical results obtained with our methodological approach, providing thus an initial validation of its utility for research.





In that context, we present several ancillary sub-contributions.
The first of these corresponds to  a tool (EdgeDroid) for the emulation of human behavior in \ac{WCA}.
This tool corresponds to a key enabler of our main contribution, as it implements the first ever parametrizable model of human behavior for these applications using empirical data collected from a series of human-subject experiments.
Furthermore, as part of the ground-laying work for this model, we present a first-ever characterization of the effects of individual personality differences in human responses to reduced system responsiveness in interactive systems such as \acl{WCA}.
...
\begin{contributions}
    \begin{enumerate}[
        ...
        \item\label{item:contrib:methodology} 
        \item In conjunction with the aforementioned methodology,
        \item A 
