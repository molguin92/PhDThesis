...
For example, the \gls{YCSB}~\cite{cooper2010benchmarking} is a data-intensive benchmark for the profiling of data centers and cloud services.
Workload characterization in datacenters leads to a similar benchmark in~\cite{jia2013characterizing}.
Both these approaches heavily focus on data processing and downlink transmission in cloud computing scenarios.
In~\cite{turner2012cmart}, the authors introduce \emph{C-MART}, a benchmark for modern web applications running on the cloud.
The benchmark utilizes modern web technologies, can be deployed automatically at a large scale, and include generators which mimic remote clients accessing the system over the internet.\@
...
A number of works have tackled this challenge in the context of Fog and Edge Computing as well.
\emph{EdgeBench}, introduced in~\cite{das2018edgebench}, corresponds to a benchmarking suite for the Edge.
It emulates three different workloads, speech-to-text, image recognition, and sensor network application traffic, and targets exclusively server and specifically targets serverless deployments.\@
\cite{baurle2022comb} presents \emph{ComB}, a flexible and extensible, application-oriented benchmarking suite for Edge computing.
Its stated purpose is to assist system designers and application developers to evaluate the suitability of Edge deployments.
The framework is designed to decouple orchestration and workload, allowing for easy extension or replacement of the built-in distributed video-analytics benchmark.
...
\cite{mcchesney2019defog} presents a benchmarking suite for the comparison of deployments on the Edge, Fog, or Cloud named \emph{DeFog}.
\emph{DeFog} includes six Fog- and Edge-relevant workloads, including real-time video detection and a text-to-speech application, which are latency-critical and bandwidth-intensive.
In~\cite{lockhart2020scission}, the authors introduce a tool, \emph{Scission}, for the benchmarking of \glspl{DNN} partitioned across the user device, edge, and cloud continuum.
The tool can also be used to automatically determine the optimal partition of a \gls{DNN}.
