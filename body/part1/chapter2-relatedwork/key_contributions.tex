\section{Overview of the contributions of this dissertation}
...
% \begin{enumerate}
%     \item A methodology for the benchmarking of latency-sensitive, feedback-loop applications enabled by Edge Computing.
%     This methodology is validated by example through representative use-cases, \gls{WCA} and \glspl{NCS}.
%     \begin{description}
%         \item[Novelty:] This is the first formalized method for this challenge in literature.
%     \end{description}

%     \item An exploration and characterization of the optimization potential of \gls{WCA} applications using the methodology, in particular with respect to sampling strategies and resulting resource and energy consumption profiles.
%     \begin{description}
%         \item[Novelty:] the study of \gls{WCA} in literature has, so far, focused primarily on prototyping applications and platforms.
%         To the best of our knowledge, only a limited subset of works~\cite{chen2017empirical,wang2019towards,wang2020scaling} deal with the performance characterization of these applications.
%         Additionally, only~\cite{wang2019towards,wang2020scaling} present heuristics for resource consumption optimization in \gls{WCA}, but no formal mathematical derivation of optimums is performed.
%     \end{description}

%     \item Tools for the implementation of the methodology, including end-to-end emulation frameworks for \gls{WCA} and \glspl{NCS} as well as a tool for Edge Computing testbed automation.
%     \begin{description}
%         \item[Novelty:] to the best of our knowledge, EdgeDroid is the first work in literature dealing with the dynamic emulation of \gls{XR} applications in networked contexts.
%         Existing works are either completely forgo the networking aspect our use rigid, deterministic workloads which fail to capture the feedback-loop interactions between system responsiveness and human behavior.
        
%         Likewise, \gls{CLEAVE} is the first tool for fully softwarized end-to-end emulation of generic \glspl{NCS} employing Edge and Cloud-native technologies.
%         Most other experimental approaches to \gls{NCS} research either fully simulate the system or employ real physical control systems.
%         Although a number of works exist employing a similar network-in-the-loop approach, they limit their functionality to specific control system use cases such as smart grid control, and do not represent general approaches.
%         They also do not employ Cloud or Edge-native technologies.

%         Finally, Ainur represents the first framework for Edge Computing testbed research with end-to-end automation and orchestration.
%         Existing approaches have focused primarily on achieving feature-parity with public clouds and targeting large-scale deployments, and have forgone the end-to-end automation aspect of Edge Computing experimentation.
%         Ainur is the first toolkit able to configure a testbed from scratch, run experiments, collect the data, and clean up afterwards in an automated fashion.
%     \end{description}
% \end{enumerate}