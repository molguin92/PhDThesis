...
\begin{abstract}
    ...
    The first contribution involves a methodological approach to experimentally studying the trade-offs between system responsiveness and resource consumption in Edge-bound latency-sensitive applications such as \glspl{CPS} and \gls{XR}.
    ...
    The approach presented in this thesis involves therefore the emulation of the client-side workload while maintaining the real server-side process and network stack to retain the realism of network effects on the system.

    The second key contribution relates to the exploration of the requirements for realism in the emulation.
    ...
    By involving client-side emulation of workloads and fully integrating workload components into the software domain, this methodology reduces complexity while still capturing complex effects of network and compute factors that are challenging to model.
    This approach represents an important contribution to literature, as it consists of a comprehensive method for the performance evaluation of Edge environments, encompassing both the application and the infrastructure.
