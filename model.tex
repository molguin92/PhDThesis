...
It showcases the important behaviors of the realistic models which are not captured by naive, non-adaptive model.
On one hand, the realistic models (and by extension, humans) are actually faster than the naive model by up to \SI{4.37}{\percent} at the lowest level of impairment.
...
On the other, the realistic models experience a slowdown of about \SI{5.13}{\percent} at the highest levels of impairment when compared to the naive model.
This is explained by the reduced system responsiveness either negating the speed-up effect, or directly leading to the \emph{additional} behavioral slowdown previously described~\cite{olguinmunoz:impact2021}.
It should be noted that for the \numlist{7;10} client setups only about \SI{6}{\percent} and \SI{22}{\percent} of \acp{TTF} fell into the medium impairment level, and practically no \acp{TTF}
