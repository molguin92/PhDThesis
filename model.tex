...
We begin by providing some formal definitions relating to step-based \acl{WCA}.
\begin{description}
    \item[Task:] understood as a finite sequence of instructions to be executed in order.
    \item[Step:] specific action to be performed by the user, described by a single instruction; e.g. put a green $1 \times 3$ brick on the board.
    \item[Step duration:] the time interval between 
    \item[Step execution time:]
    \item[Wait time:]
    \item[\ac{TTF}:]   
\end{description}

The step starts at \(t_0\), when the instruction is presented to the user.
We call the interval \(t_c - t_0\) the ``execution time'', corresponding to the time the user takes to complete the instruction.
The final sample of the step is taken at \(t_n\) and it takes \( t_{n+1} - t_n \) time units for the user to receive a reply from the backend (``last sample \ac{RTT}'').
Finally, we call the time interval between when the user finishes the instruction and the capturing of the final sample the ``wait time''; together with the last sample \ac{RTT} it composes the ``Time-to-Feedback'' of the step.%
...
A task will be understood as a finite sequence of instructions to be performed in order. Step will refer to a specific action to be performed by the user, described by a single instruction; e.g. put a green 1 × 3 brick on the board (see Fig 2). A step starts with an instruction being given to the user, and ends when the assistant detects that the user has finished performing it. At that point, the assistant either provides a new instruction to the user, or the task ends.
...
Based on these insights and the data collected for~\cite{olguinmunoz:impact2021}, we design a generator of realistic execution times for step-based \ac{WCA}.
