...
\begin{enumerate}
    ...
    The \emph{iperf3} deployment is configured to generate additional network load by constantly sending \SI{1460}{\byte} \ac{UDP} datagrams on the uplink; the rate at which these datagrams are sent is being limited by the \acs{CPU} of the Raspberry Pi client.
...
\begin{figure}
    ...
    \caption{
        ...
        We can evidence both how the realistic models speed up at with respect to the naive model when completely unimpaired, and how these same models strongly slow down as system impairment increases.
    }\label{fig:scaling_duration}
\end{figure}

\cref{fig:scaling_duration} shows the results for scenario \labelcref{verification:scaling}.
