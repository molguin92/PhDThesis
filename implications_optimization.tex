...
In this work, we use the modelling from~\cite{secAperiodic} to find the optimum aperiodic sampling interval.
However, instead of using their two-step approach to find the solution which includes a recursive solution followed by a bisection algorithm, we develop a novel, approximate, but easier solution to finding the set of the optimum aperiodic sampling intervals. Furthermore, instead of directly optimising for energy, we start by noting that an objective metric of a variety of optimisation problems that are related to sampling, RTT, responsiveness or wait time present themselves 
% in a format similar to that of the energy, 
as a linear combination between $\mathbb{E}[\mathcal{S}]$ and $\mathbb{E}[\mathcal{W}]$ plus terms independent of the number of samples or wait time. Let $\mathcal{E}$ correspond to such an objective metric. Thus,
\begin{alignat}{1}
    \Rightarrow\mathcal{E}&=\alpha\mathbb{E}[\mathcal{S}]+\beta\mathbb{E}[\mathcal{W}]+C,\;\label{eq:epsilon_terminal}
\end{alignat}
Here, $\alpha, \beta$ and $C$ are constants which are responsible for modifying the objective function from one metric to another. For instance, in the modelling used by Moothedath et.al~\cite{ICCperiodic1,TMCperiodic,secAperiodic}, the chosen characterisation results in a metric which is equal to the energy penalty. The outline of our solution that minimises \eqref{eq:epsilon_terminal} is provided below with the complete mathematical derivation in APPENDIX \ref{appx1}.
...
% \end{alignat}
Now, rewriting \eqref{eq:epsilon_terminal} in terms of $r(t)$ and minimising it gives as an optimum rate function $r^*(t)$ as
\begin{alignat}{1}
...
\end{alignat}
\subsubsection{Energy minimisation}\label{sec:aprxSol_Energy}
With the general solution available, minimising energy $E$ boils down to finding the appropriate $\alpha$ and $\beta$ that $\mathcal{E}=E$ in \eqref{eq:epsilon_terminal}. For this, we directly take the modelling from~\cite{secAperiodic} with a necessary modification in the assumption of one-way communication in all but the final sample. With feedback given even to the discarded samples in our model, we have, 
\begin{alignat}{2}
...
\subsubsection{Minimising number of samples }\label{sec:aprxSol_Energy_samples}
Apart from energy, another common optimisation metric is simply the number of samples. However, an unconstrained optimisation of the number of samples is trivial and meaningless as the solution points to a single sample at $t\!\rightarrow\!\infty$, which also takes the wait time to infinity. We look at the constrained optimisation of the expected number of samples with an upper bound $w_0$ for the expected wait. That is, $\mathbb{E}[\mathcal{W}]\!\leq\!w_0$. We show in APPENDIX \ref{appx1} that the appropriate $\alpha$ and $\beta$ for this problem satisfies the condition
\begin{alignat}{1}
