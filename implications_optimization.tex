...
Next, in~\cite{Moothedath2022Aperiodic}, the author retains the model, removes the constraint of periodicity, and finds the optimum aperiodic sampling instants $\{t_n,\,n=1,2,\dots\}$ that minimize the same energy penalty.
...
\todo[inline]{%
    Comment by Jaya:
    The presentation of the optimization for energy metric in (5) and later in Section 6.2 presenting the optimization for minimizing number of samples subject to weight penalty is done in a non-trivial way. A natural way to present it is to state the optimization problem in 6.2 first and state that the function in (5) is a Legrangian function of this optimization with alpha/beta being the Lagrangian parameter. People are well aware of the Lagrangian formulation, so at least mention that (5) is a Lagrangian function and alpha/beta is a Lagrangian parameter.

    Manuel: I've added a mention to the Lagrangian function below. Do we need anything else?
}
...
$\alpha, \beta$ and $C$ are constants which are responsible for modifying the objective function from one metric to another.
