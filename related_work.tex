...
\textcite{srinivasan2009performance} analyze a \ac{MAR} workload and identify and characterize the computational bottlenecks in the application.
...
\textcite{Wang2019Towards} develop a taxonomy for \ac{WCA} as well as strategies for workload reduction in these applications, including a novel adaptive sampling scheme with the goal of reducing the number of processed samples while maintaining application responsiveness.
\textcite{huang2021proactive} present a framework for the proactive allocation of edge cloud resources while taking into account the inherent mobility of wearable and mobile \ac{AR}.
...
In~\cite{Moothedath2021EnergyOptimal,Moothedath2022EnergyEfficient} we find the optimum periodic sampling interval which minimizes the energy tracking human progress of a specific subtask in \ac{WCA}.
This is extended into an aperiodic strategy in~\cite{Moothedath2022Aperiodic}.
