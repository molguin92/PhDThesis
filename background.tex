...
\begin{enumerate}
    ...
    \item The above effects are modulated by the \acs*{BFI}~\cite{oliver:bfi1999} trait of \emph{neuroticism}.
    This is discussed in more detail below in \cref{ssec:moderationeffects}
    
    % , as evidenced by a \ac{PCA}.
\end{enumerate}

\subsubsection{Moderation of effects by individual characteristics}\label{ssec:moderationeffects}

As mentioned above, we collected variables related to well-known individual differences.
These encompassed both the \acf{BFI}, introduced by \textcite{oliver:bfi1999}, and the \acf{ITQ}, described by \textcite{witmer1998measuring}.
...
\begin{figure}
    \centering
    \includegraphics[width=.9\columnwidth]{figs/new_model/correlation_neuro_exectime.png}
    \caption{%
        Correlation between neuroticism and the mean execution time of the last four steps in blocks of \num{12} steps subject to the same \ac{TTF} in ~\textcite{olguinmunoz:impact2021}.
    }\label{fig:neuroexectimecorrel}
\end{figure}

Neuroticism was identified as a modulating factor for the previously discussed pacing effects through a \ac{PCA}.
...
The effect of neuroticism was transversal to all \acp{TTF} and impairment durations in the tasks, and we show an example of this correlation between execution time and neuroticism in \cref{fig:neuroexectimecorrel}

Furthermore, we found that execution times were well-fit by an \ac{exGaussian} distribution, a distribution with ample body of research supporting its suitability for the modeling of the timing of human actions and reaction times~\cite{Rohrer1994analysis,Palmer2011shapes,Marmolejo2022generalised}.
