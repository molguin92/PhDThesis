...
%DIF DEL paper_old.tex   Tue Feb 16 11:48:29 2021
%DIF ADD paper.tex       Tue Feb 16 11:47:32 2021
...
%DIF INVISIBLE PREAMBLE %DIF PREAMBLE
...
%DIF DIFCODE_INVISIBLE %DIF PREAMBLE
...
Furthermore, this effect persists for a while after system response improves and is modulated by intrinsic personality traits, in particular, \emph{neuroticism}~}%DIFDELCMD < \cite{john1999:bfi}%%%
\DIFdel{, which has previously been connected to intolerance for time delay~}%DIFDELCMD < \cite{hirsh2008delay}%%%
\DIFdel{. 
...
%DIF >  Our results indicate that reduced responsiveness in WCA systems leads to a disruption of participants' cognitive plan for the task.
%DIF >  This is evidenced by an emergent pacing effect on user actions as system responsiveness is reduced.
%DIF >  While it would seem self-evident that users take longer to complete a task while using a system with low responsiveness --- as they have to wait longer for new instructions --- our study found that user slow-down represents a source of substantial additional delay.
%DIF >  To be more precise, the data indicate that users slow down not only because they have to wait for the system to catch up, but that their reactions to new instructions is also delayed.
%DIF >  Furthermore, this effect persists for a while after system response improves and is modulated by intrinsic personality traits, in particular, \emph{neuroticism}~\cite{john1999:bfi}. 

\DIFaddend We believe that these results provide concrete and relevant implications for WCA design, deployment, and optimization.
...
%DIF >  Overarching theo construct: people perform temp seq task in the framework of cognition and emotion and individual differences mediate response to delay. In particular we view cognition as described by models such as ACT-R as a seq of procedures.

%DIF >  Section 2.
%DIF >  Accordingly we review time perception...

%DIF >  Our overarching theoretical construct is the idea that people perform temporally sequential tasks in the framework of cognition and emotion.
%DIF >  Additionally, we hypothesize that individual diferences between subjects moderate responses to delays in the feedback during the execution of a task.
%DIF >  In particular, we view cognition as described by models such as {ACT-R}~\cite{neves1981knowledge}; i.e.\ as a sequence of procedures performed by the subject.
...
In particular, we view cognition as described by models such as }{\DIFadd{ACT-R}}\DIFadd{~}\cite{neves1981knowledge}\DIFadd{; i.e.\ as a sequence of procedures performed by the subject.
...
A set of instructions are to be performed by }\DIFdelend %DIF >  Such applications constitute ``\emph{conversational computing tasks}'' in the taxonomy proposed by \textcite{dabrowsky:2011:40years}.
...
%DIF >  The results of the user performing these instructions are provided as inputs to the system.
%DIF >  These inputs may either be correct, in which case the system proceeds to output the next instruction, or incorrect, in which case a procedurally generated corrective step that fixes the mistake is provided to the user.
}\DIFaddend 
...
}%DIFDELCMD < \theoremstyle{definition}
%DIFDELCMD < \begin{definition}[Task and subtask]
%DIFDELCMD <   %%%
\DIFdelend %DIF >  In more formal terms, we can provide definitions for \emph{task}, \emph{subtask}, and \emph{step} in such an application as:
...
%DIF >  \begin{definition}[Task and subtask]
...
%DIF >  We will refer to the interval of time delimited by two consecutive instructions (see \cref{fig:task:steps,fig:cogassist:step}), characterized by the actions of both the user and the assistant.
...
\DIFdelend \DIFaddbegin \DIFadd{to the user, or the task ends.
  %DIF >  See for instance \cref{fig:task:steps}, which pictures the task of assembling a simple LEGO model, with each subtask corresponding to the addition of a specific LEGO piece to the current model.  
%DIF >  \end{definition}
}\DIFaddend 
...
%DIF >  \begin{definition}[Step]

%DIF >  While the subtask remains unfinished, the assistant discards results of the processing of the sampled inputs.
%DIF >  At the beginning of a step, an instruction is given to the user.
%DIF >  The user then proceeds to perform the subtask specified by the instruction, while the cognitive assistant is continuously sampling the subtask state at specified intervals. 

%DIF >  Once the user finishes the required action, the next sample taken will contain a valid input, and thus the cognitive assistant will provide a new instruction. 
%DIF >  This finishes the current step and potentially begins a new one if there remain instructions to be performed in the task.
%DIF >  \end{definition}
...
%DIFDELCMD <     \item \ It is easily understood by users and requires essentially no training. 
%DIFDELCMD <     \item\ The step-by-step nature of these applications simplifies the isolation of the relevant experimental variables and the effects of the delay.
%DIFDELCMD <     \item The states of the display can be recognized by simple image processing algorithms and do not require extensive training as for machine-learning-based applications.
%DIFDELCMD <     \item Each step has an intrinsic hierarchical cognitive structure (see \cref{fig:lego:hierarchical}), affording multiple levels of cognitive control. 
...
\begin{figure*}[h]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \input{plots/lego_hierarchy_diagram.tex}
%DIFDELCMD <   %%%
\DIFdelendFL %DIF >  \input{plots/lego_hierarchy_diagram.tex}
  % \DIFaddbeginFL \includegraphics[width=\textwidth]{plots/lego_hierarchy_diagram_standalone.eps} \DIFaddendFL
...
}\DIFdelend %DIF >  The original design of the LEGO assistance application was based on a client-server model communicating over a wireless network, with the client software running on a wearable device and the server software deployed on a cloudlet. 
...
%DIF >  By this means, we achieve much more fine-grained control over the latencies to which the system is subject.
%DIF >  Additionally, this greatly simplified the instrumentation of the application.
...
}\DIFdelend %DIF >  Finally, a data collection and experimental layer was implemented between the user interface and video capture and the core processing component of the LEGO Assistant.
%DIF >  This layer controlled the manipulated experimental variables and recorded the measured biometric and task-related effects.
...
Thus, our }\DIFdelend %DIF >  A value of  \SI{600}{\milli\second} was identified as the bound where users start noticing delays in the assistant.
%DIF >  Conversely, \SI{2.7}{\second} was identified as the upper bound on delays after which the application is considered to be in such a degraded state it is basically ``unusable''.
...
%DIF >  while including one delay value in the unnoticeable range and one completely in the ``unusable'' range.
}\DIFaddend 
...
%DIF >  \begin{definition}[Block]
%DIF >    A sequence of consecutive steps within a task subject to the same delay; see \cref{fig:cogassist:block}.
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  ; e.g.\ for \cref{fig:cogassist:block}, the duration of the pictured block would be \( t_{n+k} - t_{n} \).
%DIF >  \end{definition}
...
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=.8\textwidth]{images/step_structure.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Structure of a step in a generic cognitive assistance application. 
    ...
    However, once the user finishes performing the given instruction, the next input \emph{will} cause the generation of a new instruction, which will subsequently be provided to the user.}}%DIFAUXCMD
%DIF < 
    %DIFDELCMD < \label{fig:cogassist:step}
%DIFDELCMD <   \end{subfigure}
%DIFDELCMD <   \medskip%%%
%DIF < 
  %DIFDELCMD < \begin{subfigure}{\textwidth}
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=.8\textwidth]{images/step_structure_delay.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Modified structure of a step in the experimental task. In contrast to \cref{fig:cogassist:step}, an additional variable segment of time is introduced immediately following the processing of the input frame in the cognitive assistant, in order to extend the perceived processing time of the input to a specific target delay.}}%DIFAUXCMD
%DIF < 
    %DIFDELCMD < \label{fig:cogassist:step:delay}
%DIFDELCMD <   \end{subfigure}
%DIFDELCMD <   \medskip%%%
%DIF < 
  %DIFDELCMD < \begin{subfigure}{\textwidth}
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=.8\textwidth]{images/block.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Structure of a block in the experimental task.}}%DIFAUXCMD
%DIF < 
    %DIFDELCMD < \label{fig:cogassist:block}
%DIFDELCMD <   \end{subfigure}%%%
%DIF < 
  %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Components of the cognitive assistance task.}}
%DIFAUXCMD
...
}\DIFdelend %DIF >  The design rotates the block types (as defined by length and delay) across participants so that each type is tested in each ordinal position, but it only coarsely samples from the \( 21 \times 20 \) possible sequences from one block type to another.
...
%DIF >  Users were directed to either add a piece to the ongoing model or to remove a piece, and blocks were designed in such as way so as to ensure that the transitions between them were invisible to the user. 
}\DIFaddend 
...
}\DIFdelend %DIF >  In this paper we will thus consider blocks to be our basic element of study, and most aggregations will be done at this level (with a few exceptions).
%DIF >  For this, we will need additional definitions:

\DIFdelbegin %DIFDELCMD < \begin{definition}[Delay associated with a block]
%DIFDELCMD <   %%%
\DIFdel{We will refer to the delay of a block as the delay applied to every frame of every step in that block.
}%DIFDELCMD < \end{definition}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{definition}[Delay associated with a block]
%DIF >    We will refer to the delay of a block as the delay applied to every frame of every step in that block.
%DIF >  \end{definition}

\DIFdelbegin %DIFDELCMD < \begin{definition}[Execution time]
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{definition}[Execution time]
...
%DIF >  their presenting the completed subtask to the system.
}\DIFaddend 
...
}\DIFdelend %DIF >  This is also aggregated into an average at the block level.

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <     %%%
\DIFdelendFL %DIF >  \begin{figure}
%DIF >    \centering
%DIF >    \includegraphics[width=.85\textwidth]{images/exec_time_diagram.pdf}
%DIF >    \caption{Visualization of the execution time of a step.}%
%DIF >    \label{fig:exectime:diagram}
%DIF >  \end{figure}
\DIFaddbeginFL 
...
\begin{figure}[h]
  ...
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.85\textwidth]{images/exec_time_diagram.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[t]{.49\textwidth}
    ...
    %DIF >  by the assistant, as they do not contain relevant information.
    Once the user finishes performing the step, the next sample \emph{will} cause the generation of a new instruction.
    %DIF >  which will subsequently be provided to the user.}%
    } \DIFaddend}
  ...
  \begin{subfigure}[t]{.49\textwidth}
    ...
    \caption{\DIFaddFL{In the experimental task, an additional variable segment of time is introduced immediately following the processing of the input frame in order to extend the perceived processing time of the input to a specific target delay.}}%DIF > 
  ...
  \begin{subfigure}[t]{.49\textwidth}
    ...
    \caption{\DIFaddFL{Structure of a block in the experimental task.}}%DIF > 
  ...
  \DIFaddbeginFL \end{subfigure}%DIF > 
  %DIF >  \medskip%
...
%DIFDELCMD <   \item galvanic skin response (\emph{GSR});
%DIFDELCMD <   \item accelerometer data from the dominant wrist;
%DIFDELCMD <   \item brain activity in the form of electroencephalography (\emph{EEG});
%DIFDELCMD <   \item and heart rate.
...
\DIFdelbegin \DIFdel{Following capture, the  EEG signal was post-processed in the following fashion:
%DIF <  
}%DIFDELCMD < \begin{enumerate}
%DIFDELCMD <   \item Since our main interest was in the \( \alpha \) (\SIrange{8}{12}{\hertz}) and \( \beta \) (\SIrange{12}{30}{\hertz}) bands, a low-pass Butterworth filter of order 8 and cutoff frequency \SI{40}{\hertz} was applied to filter out noise in the higher end of the spectrum (in particular, noise from the board power supply at \SI{60}{\hertz}).
%DIFDELCMD <   \item A high-pass Butterworth filter of order 8 and cutoff frequency \SI{0.1}{\hertz} was then applied to filter out noise at the low end of the spectrum.
%DIFDELCMD <   \item Subsequently, a pair of cascaded Savitsky-Golay filters~\cite{savitzky1964smoothing}, both of order 8 and window size 21, were applied to the signal in order to smooth out noise, as proposed by \textcite{agarwal2017eeg}.
%DIFDELCMD <   \item A spectrogram was then calculated for the signal.
%DIFDELCMD <   \item Finally, the power over time for each EEG frequency band is obtained by integrating over the relevant frequencies for each time-step in the spectrogram.
%DIFDELCMD < \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{An example of the effects of the filtering on the raw EEG signal can be observed in \cref{fig:eeg:raw_vs_filt}.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=.8\textwidth]{plots/raw_vs_filt_eeg.eps}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Comparison of an EEG signal before and after filtering.
  Note in particular the artifact from the DC power supply of the capture board around \SI{60}{\hertz} in the raw signal, and how it essentially disappears after filtering.}}%DIFAUXCMD
%DIF < 
  %DIFDELCMD < \label{fig:eeg:raw_vs_filt}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsubsection{Video recordings}
...
We confirmed this effect through an \emph{analysis of variance} (ANOVA\DIFaddbegin \DIFadd{~}\cite{FUJIKOSHI1993ANOVA}\DIFaddend ) with factors of block length and delay\DIFdelbegin \DIFdel{.
...
The current length X delay ANOVA }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend found significant main effects of both factors and the interaction, shown in \cref{tab:anova:exectime}.
%DIF >  An ANOVA test uses the \emph{F-test} statistic, which is a ratio of the variability in the data introduced by experimental manipulations (and their interactions) to the variability in the data attributable to randomness.
%DIF >  The degrees of freedom represent the number of observations going into each of these variability measures.
%DIF >  As we are using within-subject designs, the variability in the data due to randomness is estimated by the differences among subjects with respect to the effects of the experimental variables.
%DIF >  The partial-\( \eta^2 \) (\(\eta_{p}^{2}\)) statistic is a measure of effect size, corresponding to the proportion of variance explained by the effect after excluding variance from others, and \( p \)-value, a measure of the probability of the effect under the null hypothesis.
%DIF >  Conventionally \( p < .05 \) is the criterion for significance.
%DIF >  The current length \( \times \) delay ANOVA found significant main effects of both factors and the interaction, shown in \cref{tab:anova:exectime}.

%DIF >  \begin{figure}[h]
%DIF >      \centering
%DIF >      \includegraphics[width=.8\textwidth]{plots/exec_time_delay_length.eps}
%DIF >      \caption{Per-step execution time by block length vs.\ delay. Error bars indicate the Standard Error of the Mean (S.E.M.)}\label{fig:exectime}%    
%DIF >  \end{figure}
\DIFaddbegin 
...
%DIF >  physiological measures of GSR and HR failed to show evidence of differential arousal under long vs.\ short delays, and speed-induced errors and non-completions predicted by resource depletion were not observed.
%DIF >  The acceleration data further do not indicate that extended delay increases erratic movement.
...
%DIFDELCMD <   \item Due to the behavioral slow-down in users, even short-term reductions in responsiveness will lead to significantly extended application lifetimes.
%DIFDELCMD <   This has direct implications for resource and power consumption.
%DIFDELCMD <   

%DIFDELCMD <   \item The fact that the adverse effects of delay on users do not immediately disappear as the system returns to a high-responsive state could have unconventional consequences for resource allocation.
%DIFDELCMD <   This is of particular importance, for instance, for cases where the user may be able to finish the task before these effects subside.
%DIFDELCMD <   In such cases, the limited potential gains might not justify diverting valuable system resources to the impaired application.
%DIFDELCMD < 

%DIFDELCMD <   \item In multi-user environments, the time dependency of user slow-down effects mean that fair degradation of system responsiveness across applications may not ultimately be beneficial to the system as a whole.
%DIFDELCMD <   Take for instance two applications on the same system which negatively interfere with each other. 
%DIFDELCMD <   The longer they interfere with each other, the longer their respective lifetimes are going to be, which in turn causes them to interfere even longer, potentially entering a negative feedback loop. 
%DIFDELCMD <   In such a case, prioritizing one over the other rather than trying to improve responsiveness for both might lead to resources being freed up faster system-wide.
%DIFDELCMD < 

%DIFDELCMD <   \item Based on our findings relating individual differences between users and their sensitivity to delays, it might also be possible to extrapolate user characteristic from measured execution times.
%DIFDELCMD <   This could prove a valuable tool for load balancing, for instance by prioritizing resource allocation to users with a higher sensitivity to system-state degradation.
%DIFDELCMD <   However, this remains an open challenge.
%DIFDELCMD < 
