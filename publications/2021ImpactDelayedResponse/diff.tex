...
Furthermore, this effect persists for a while after system response improves and is modulated by intrinsic personality traits, in particular, \emph{neuroticism}~}%DIFDELCMD < \autocite{john1999:bfi}%%%
\DIFdel{, which has previously been connected to intolerance for time delay~}%DIFDELCMD < \autocite{hirsh2008delay}%%%
\DIFdel{. 
...
%DIF >  Furthermore, this effect persists for a while after system response improves and is modulated by intrinsic personality traits, in particular, \emph{neuroticism}~\autocite{john1999:bfi}. 
...
%DIF >  In particular, we view cognition as described by models such as {ACT-R}~\autocite{neves1981knowledge}; i.e.\ as a sequence of procedures performed by the subject.
...
%DIFDELCMD <   \item Subsequently, a pair of cascaded Savitsky-Golay filters~\autocite{savitzky1964smoothing}, both of order 8 and window size 21, were applied to the signal in order to smooth out noise, as proposed by \textcite{agarwal2017eeg}.
...
We confirmed this effect through an \emph{analysis of variance} (ANOVA\DIFaddbegin \DIFadd{~}\autocite{FUJIKOSHI1993ANOVA}\DIFaddend ) with factors of block length and delay\DIFdelbegin \DIFdel{.
