...
Types of connections handled by this layer could be ethernet and WiFi links, or even an entire software-defined 4G \gls{LTE} network.
...
Currently, WiFi and \gls{LTE} implementations are available.
...
\begin{description}[style=nextline]
    ...
    It searches for an WiFi \gls{SSID} among the networks that are being advertised and attempts for establishing the connection.
    ...
    Unlike WiFi wireless connection, due to the fact that \gls{LTE} creates an internal ip network and that introduces routing complexities, the server host and the client host cannot be on the same subnet in the ip layer.
    

    \begin{figure}[t]
        \centering
        \includegraphics[width=0.8\linewidth]{figures/lte.png}
        \caption{An example of \gls{LTE} data network topology in data network and its corresponding \acp{VLAN}}
        \label{fig:lte}
    \end{figure}
