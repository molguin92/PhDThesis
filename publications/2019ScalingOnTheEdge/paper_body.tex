...
\Cref{sec:background} presents some background on human-in-the-loop applications.
\Cref{sec:approach} discusses the general approach taken with EdgeDroid, while \Cref{sec:implementation} exposes the implementation details for EdgeDroid 1.0.
We show the value of the tool through use case analysis in \Cref{sec:usecases,sec:results}, before discussing future work and concluding in \Cref{sec:conclusions}.
...
Given the wide range of applications which fall under this concept, we have chosen to focus our initial efforts on one particular category: task-guidance wearable cognitive assistance (WCA)~\cite{Ha:TowardsWearableCogAssist}.
...
\begin{figure}[b]
    \centering
    \input{img/pipeline.tex}
    \caption{Pipeline of human-in-the-loop applications.}\label{fig:pipeline2}
\end{figure}

The task model of a task-guidance WCA is a parameterized representation of the task in question and the steps required to complete it.
...
\begin{figure}[tb]
    \centering
    \input{img/taskmodel.tex}
    \captionof{figure}{Segment of the internal task model of a linear task-guidance WCA.}%
    \label{fig:taskmodel}
\end{figure}
...
They replay this data over the network to the \emph{application instances} employing, as mentioned in \Cref{sec:background}, a \emph{user model}, while simultaneously collecting client-side statistics and measurements.
...
For our initial iteration of the EdgeDroid 1.0 benchmarking suite, we implement a preliminary user model, depicted in \Cref{fig:usermodel}, to run on the \emph{client emulators}, designed with a linear task model in mind.
...
\begin{figure}[tb]
    ...
    \captionof{figure}{Preliminary user model.}\label{fig:usermodel}
\end{figure}
...
The application employs a straightforward, linear task model as the one described in \Cref{sec:background}.
...
\Cref{tbl:cloudletclienthardware} shows the hardware and software specifications for the cloudlet and the clients.
